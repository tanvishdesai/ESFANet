...
class Config:
    ...
    NON_IID_SCENARIO = 'B' # Change this value to switch scenarios
...
def create_non_iid_splits_from_dataset(dataset, num_clients, config):
    ...
    """
    # MODIFIED: Select alpha based on the scenario in the config
    scenario_alphas = {
        'A': 10.0, # Low Non-IID
        'B': 1.0,  # Moderate Non-IID
        'C': 0.5   # Pathological Non-IID
    }
    ...
    for c in range(num_classes):
        ...
        num_samples_per_client[-1] += len(class_indices) - sum(num_samples_per_client)
        start_idx = 0
