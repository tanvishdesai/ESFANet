# -*- coding: utf-8 -*-
"""
EFSANetV2 (Ablation Study: Edge Attention Only): Breast Cancer Histopathology 
Classification with a Hybrid Attention Model built on a Pretrained Backbone.

This script implements an ablation study of the EFSANetV2 model.

Ablation Study: Backbone + Edge Attention Only + Transformer
- The Frequency-Aware Channel Attention part of the EdgeFrequencyAttention
  module has been removed to isolate the impact of the edge-aware spatial attention.
"""
import os
import numpy as np
import pandas as pd
...
# 2.1 Custom Attention Module: Edge-Frequency Attention
# <--- MODIFICATION FOR ABLATION STUDY --- >
# The Frequency-Aware Channel Attention part has been removed.
class EdgeFrequencyAttention(nn.Module):
    def __init__(self, in_channels):
        super(EdgeFrequencyAttention, self).__init__()
        # Edge-Aware Spatial Attention (This part is kept)
        ...
        # <--- MODIFICATION FOR ABLATION STUDY --- >
        # Frequency-Aware Channel Attention has been removed.
    ...
    def forward(self, x):
        # Spatial Attention (This part is kept)
        ...
        # <--- MODIFICATION FOR ABLATION STUDY --- >
        # Channel Attention calculation is removed.
        ...
        # <--- MODIFICATION FOR ABLATION STUDY --- >
        # Combine attention: Only spatial attention is used now.
        # The original line was: x_att = x * spatial_attention * channel_attention
...
# 2.3 The Full Revised EFSANet Model (Unchanged logic, but now uses the modified attention)
class EFSANet(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        # This will now instantiate the modified EdgeFrequencyAttention module
...
def main():
    ...
    print("\nStep 2: Initializing the EFSANet model for Ablation Study (Edge Attention Only)...")
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_f1_score:
            ...
            torch.save(model.state_dict(), 'best_model_ablation_edge_only.pth')

    print("\nTraining finished.")
    
    if os.path.exists('best_model_ablation_edge_only.pth'):
        print("\nStep 4: Performing final evaluation on the test set...")
        model.load_state_dict(torch.load('best_model_ablation_edge_only.pth'))
