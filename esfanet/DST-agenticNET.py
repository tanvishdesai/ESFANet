...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch):
    ...
    # We define a "Target" distribution: We want agents to be roughly [0.33, 0.33, 0.33]
    # But we won't force it hard, we just nudge the model towards it.
    target_weights = torch.tensor([1.0/3, 1.0/3, 1.0/3], device=device)
    
    # Increase this alpha to force more sharing. 
    # 0.5 is usually enough to get the graph to look mixed without ruining accuracy.
    DIVERSITY_ALPHA = 0.5 

    for inputs, labels in tqdm(dataloader, desc="Training (w/ Diversity Reg)", leave=False):
        ...
        # --- 1. Main Task Loss (EDL) ---
        loss_main = criterion(fused_evidence, labels)
        
        # --- 2. Auxiliary Loss (Deep Supervision) ---
        # Keep the weak agents learning
        loss_aux = 0.2 * (criterion(individual_evidences[:,0,:], labels) + 
                          criterion(individual_evidences[:,1,:], labels) + 
                          criterion(individual_evidences[:,2,:], labels))
        
        # --- 3. DIVERSITY REGULARIZATION (The Fix for the Graph) ---
        # weights shape: [Batch, 3, 1]. Squeeze to [Batch, 3]
        w = weights.squeeze()
        
        # Calculate how far the weights are from being equal
        # This penalizes [1, 0, 0] and encourages [0.8, 0.1, 0.1]
        loss_div = F.mse_loss(w, target_weights.expand_as(w))
        
        # Total Loss
        loss = loss_main + loss_aux + (DIVERSITY_ALPHA * loss_div)
