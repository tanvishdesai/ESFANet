# -*- coding: utf-8 -*-
"""
EFSANetV2 (Ablation Study): Backbone + Full Attention (No Transformer)

This script implements an ablation study for the EFSANetV2 model.
The goal is to evaluate the performance of the DenseNet121 backbone combined
with only the EdgeFrequencyAttention module, followed by Global Average Pooling.

This setup tests the independent contribution of the novel attention mechanism
without the sequential processing of the Transformer Encoder.
"""
import os
import numpy as np
import pandas as pd
...
# --- Configuration ---
DATA_DIR = '/kaggle/input/breast-histopathology-images' # Changed for local testing, adjust if needed
IMAGE_SIZE = 96
BATCH_SIZE = 128
EPOCHS = 30
LEARNING_RATE = 1e-4
...
# 2.2 NEW ABLATION MODEL: Backbone + Attention + Pooling
class AblationAttentionOnly(nn.Module):
    """
    Ablation Study Model: DenseNet121 Backbone, EdgeFrequencyAttention,
    and Global Average Pooling.
    """
    def __init__(self, num_classes=2, pretrained=True):
        super(AblationAttentionOnly, self).__init__()
    ...
    def forward(self, x):
        ...
        pooled_features = self.pool(attended_features)
...
def main():
    ...
    # --- Create dummy data if DATA_DIR does not exist ---
    if not os.path.exists(DATA_DIR):
        print("Dummy data directory not found. Creating dummy data for demonstration...")
        os.makedirs(os.path.join(DATA_DIR, '0'), exist_ok=True)
        os.makedirs(os.path.join(DATA_DIR, '1'), exist_ok=True)
        for i in range(200): # Create a small dummy dataset
            Image.new('RGB', (50, 50)).save(os.path.join(DATA_DIR, '0', f'dummy_{i}_class0.png'))
        for i in range(200):
            Image.new('RGB', (50, 50)).save(os.path.join(DATA_DIR, '1', f'dummy_{i}_class1.png'))
    ...
    print("\nStep 2: Initializing the ABLATION STUDY model (Backbone + Attention only)...")
    # MODIFICATION: Instantiate the ablation model instead of the full EFSANet
    model = AblationAttentionOnly(num_classes=2).to(DEVICE)
    
    class_counts = np.bincount(train_df['label'])
    if len(class_counts) < 2: # Handle case with only one class in a small dummy set
        class_counts = np.array([len(train_df)//2, len(train_df)//2])
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_f1_score:
            ...
            torch.save(model.state_dict(), 'best_model_ablation_attention_only.pth')

    print("\nTraining finished.")
    
    if os.path.exists('best_model_ablation_attention_only.pth'):
        print("\nStep 4: Performing final evaluation on the test set...")
        model.load_state_dict(torch.load('best_model_ablation_attention_only.pth'))
        
        test_loss, test_acc, test_prec, test_rec, test_f1 = evaluate(model, test_loader, criterion, DEVICE)
        
        print("\n--- Test Set Results (Ablation: Attention Only) ---")
